        ...
        <div v-if="result != {}">
            <div v-for="(value, key, index) in result" v-bind:key="index">
                <div v-if="key=='tasks'">
                    ...
                    <ResultElement v-for="(taskItem, lIndex) in value" v-bind:key="lIndex" :item="taskItem" :type="'task'" @close="close"/>
                </div>
                <div v-else-if="key=='boards'">
                    <span v-if="value.length !== 0 && key=='boards'">Boards: </span>
                    <ResultElement v-for="(boardItem, lIndex) in value" v-bind:key="lIndex" :item="boardItem" :type="'board'" @close="close"/>
                </div>
            </div>
        </div>
    </div>
